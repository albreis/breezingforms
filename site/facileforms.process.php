...
// header
	...
	function cbCreatePathByTokens($path, array $rows, $field_name){
		...
		foreach($rows As $row){
			$value = JRequest::getVar( 'ff_nm_' . $row->name, array(), 'POST', 'HTML', JREQUEST_ALLOWHTML );
		...
		if ($this->editable && $cbRecord === null) {
			...
			if (count($recordsResult) != 0) {
				...
				foreach ($recordEntries As $recordEntry) {
                    ...
                    JRequest::setVar('bfCleanVar', $recordEntry->value, 'POST');
                    $recordEntry->value = JRequest::getVar('bfCleanVar', '', 'POST', 'HTML', JREQUEST_ALLOWHTML);
		...
		if ($cbRecord !== null) {
			...
			foreach ($cbRecord As $cbEntry) {
				if (!in_array($cbEntry->recElementId, $cbNonEditableFields)) {
                    ...
                    JRequest::setVar('bfCleanVar', $cbEntry->recValue, 'POST');
                    $cbEntry->recValue = JRequest::getVar('bfCleanVar', '', 'POST', 'HTML', JREQUEST_ALLOWHTML);
			...
			if(version_compare($version->getShortVersion(), '3.1', '>=') && $this->formrow->tags_content != ''){
				...
				$db->setQuery("Insert Into 
                    ...
                    Values 
                        (
                          ".$db->quote($title).",
                          ".$db->quote(bf_stringURLUnicodeSlug(JRequest::getInt('Itemid', 0).'-'.$title)).",
                          ".$db->quote($introtext).",
                          ".$db->quote($fulltext).",
...
// sendMail
	...
	function exppdf($filter = array(), $mailback = false, $translate = true) {
        ...
        if(version_compare($_version, '3.2', '>=')){
            ...
            $date_stamp2 = $date_->format('Ymd', true);
        }
        ...
        if(isset($matches_array[1]) && isset($matches_array[1][0]) && trim($matches_array[1][0]) == 'fm'
            && isset($matches_array[2]) && isset($matches_array[2][0])){
	...
	function saveUpload($filename, $userfile_name, $destpath, $timestamp, $useUrl = false, $useUrlDownloadDirectory = '',$resize_target_width=0,$resize_target_height=0,$resize_type='',$resize_bgcolor='#ffffff',$field_name = '') {
		...
		if($fmtest != basename($_baseDir)){
			...
			foreach($this->rows As $row){
				$fname = JRequest::getVar( 'ff_nm_' . $row->name, array(), 'POST', 'HTML', JREQUEST_ALLOWHTML );
					...
					switch ($row->type) {
															...
															if ($parts[count($parts) - 4] == $row->name) {
																...
																if($fmtest != basename($_baseDir)){
																	...
																	foreach($this->rows As $row2)
																	{
																		...
																		$fname = JRequest::getVar( 'ff_nm_' . $row2->name, array(), 'POST', 'HTML', JREQUEST_ALLOWHTML );
						...
						case 'Signature':
							if ($row->logging == 1) {
								...
								$values = @JRequest::getVar("ff_nm_" . $row->name, array(''), 'POST', 'HTML', JREQUEST_ALLOWHTML);
									...
									if(trim($this->formrow->template_code_processed) == 'QuickMode'){
										...
										if($qmelement !== null && isset($qmelement['properties']['is_html']) && $qmelement['properties']['is_html']){
											...
											for($html_i = 0; $html_i < $html_value_cnt; $html_i++){
                                                ...
                                                JRequest::setVar('bfCleanVar', $values[$html_i], 'POST');
                                                $values[$html_i] = JRequest::getVar('bfCleanVar', '', 'POST', 'HTML', JREQUEST_ALLOWHTML);
                                            }
										}
									}
								}
...
.thebutton {
						...
						foreach ($areas As $area) {
							...
							foreach ($area['elements'] As $element) {
								...
								if ($element['internalType'] == 'bfPayPal') {
									...
									$html .= "<input type=\"hidden\" name=\"no_shipping\" value=\"" . ( isset($options['shipping']) && $options['shipping'] == 1 ? 0 : 1 ) . "\"/>";
    ...
    /**
     * Used to for editable forms.
     * BF sanitizes incoming unsafe html but we cannot rule out that the value may be changeed through 3rd parties in the database.
     * Therefore we sanitize once more on output.
     */
