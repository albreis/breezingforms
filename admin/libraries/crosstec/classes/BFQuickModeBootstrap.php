				...
				switch($mdata['bfType']){
                                                        ...
                                                        JQuery(document).ready(
                                                            function() {
                                                                ...
                                                                var uploader = new plupload.Uploader({
                                                                        ...
                                                                        url : '".$base.(BFJoomlaConfig::get('config.sef') && !BFJoomlaConfig::get('config.sef_rewrite') ? 'index.php/' : '').(JRequest::getCmd('lang','') && BFJoomlaConfig::get('config.sef') ? ( BFJoomlaConfig::get('config.sef_rewrite') ? 'index.php' : '' ) : 'index.php')."',
					...
					case 'bfReCaptcha':
						...
						if(isset($mdata['pubkey']) && $mdata['pubkey'] != ''){
							...
							} else if (isset($mdata['invisibleCaptcha']) && $mdata['invisibleCaptcha']) {
					...
					case 'bfCalendar':
						...
						if($this->rootMdata['themebootstrapThemeEngine'] == 'bootstrap' && $this->rootMdata['themebootstrap'] == 'Azure') {
							if(!isset($mdata['icon']) || $mdata['icon'] == '') {
								$icon = '<i class="fa fa-calendar iconf--fumi" aria-hidden="true"></i>';
							} else {
								$icon = '<i class="fa '.htmlentities($mdata['icon'], ENT_QUOTES, 'UTF-8').' iconf--fumi" aria-hidden="true"></i>';
							}
						}
						...
						if (version_compare($version->getShortVersion(), '3.7', '>=')) {
							...
							echo '<div class="field-calendar '.$this->bsClass('input-append').'">' . "\n";
								echo '<div class="'.$this->bsClass('input-append').'">' . "\n";
									echo '<input type="text"
											id="ff_elem' . $mdata['dbId'] . '"
											name="ff_nm_' . $mdata['bfName'] . '[]"
											autocomplete="off"
											class="ff_elem ' . $this->bsClass("form-control"). ' ' . $this->bsClass("custom-form-control") . '"
									...
									echo '<span class="'.$this->bsClass('input-group-btn').'">';
										echo '<button type="button"
											...
											class="bfCalendar '.$this->bsClass('btn').' '.$this->bsClass('btn-primary').' button"
											...
											<span><i class="'.$this->bsClass('icon-calendar').'"></i>' . htmlentities( $right == '...' ? '' : $right, ENT_QUOTES, 'UTF-8' ) . '</span>
										</button></span>' . "\n";
									echo '</div></div></span></div></div>'."\n";
							...
							.calendar-container { text-align: center; }</style>';	
